...
export function SlideTransition<K extends string>({
  ...
  const { currentScene, nextScene, staticScene } = useMemo(() => {
    const currentScene = scenesRef.current[displayedKey];
    const nextScene = transitionKey ? scenesRef.current[transitionKey] : null;
    const staticScene = scenesRef.current[activeKey];
    return { currentScene, nextScene, staticScene };
  }, [activeKey, displayedKey, transitionKey]);
